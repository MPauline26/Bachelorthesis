...
The analysed variables are listed below and they are categorized by their type of particles. 

\begin{itemize}
  \item B-jet: variables of jets with b-tag
...
The pre- or suffix of the name reveal what information is saved. The description of variables, which don't follow this logic, are listed at the end. Additionally for jet variables, 0 refers to the jet with the highest $p_T$ and 1 to the second highest $p_T$.

Prefix and suffix:
\begin{itemize}
	  \item \_eta: pseudorapidity of variable ($\eta = -ln(tan(\frac{\vartheta}{2}))$; wheras $\vartheta$ is the polar angle)
...
\begin{itemize}
  ...
  \item MET: Missing transverse momentum
		...
		\item JetGood0\_chHEF: proportion of energy coming from charged particles detected in HCAL (e.g. charged meson, proton)
  		\item LeptonTight0\_pfRelIso03\_all: relative isolation of the lepton to all particles in a radius of dR = 0.3 %$\sum_{i} (\frac{p_{T_i}}{p_{T_i}^{LEPTON}})$
		\item LeptonTight0\_pfRelIso03\_chg: relative isolation of the lepton to charged particles in a radius of dR = 0.3 %$\sum_{i} (\frac{p_{T_i}^{CHARGED}}{p_{T_i}^{LEPTON}})$
		\item LeptonTight0\_pfRelIso03\_ne: relative isolation of the lepton to neutral particles in a radius of dR = 0.3 %$\sum_{i} (\frac{p_{T_i}^{NEUTRAL}}{p_{T_i}^{LEPTON}})$
